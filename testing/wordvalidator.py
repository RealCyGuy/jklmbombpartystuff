import asyncio
import random
import glob
import re

import socketio
import requests

from utils import create_token, cleanup_word


class Human:
    def __init__(self, token, url, name):
        print(f"Created {name}!")

        log = False
        # if name == "Human 0":
        #     log = True
        self.sio = socketio.AsyncClient(logger=log, engineio_logger=log)
        self.gameSio = socketio.AsyncClient(logger=log, engineio_logger=log)
        self.token = token
        self.url = url
        self.name = name
        self.joinData = {}

        self.rules = {}
        self.milestone = {}
        self.peerId = -2
        self.leaderPeerId = -1

        self.bonus_letters = []
        self.attempts = 0

    async def join_room_callback(self, joinData):
        self.joinData = joinData
        await self.gameSio.emit("joinGame", ("bombparty", code, self.token))

    async def connect_and_join(self):
        await self.events()
        await self.sio.connect(url=self.url, transports=["websocket"])
        await self.gameSio.connect(url=self.url, transports=["websocket"])
        await self.sio.emit(
            "joinRoom",
            {
                "roomCode": code,
                "userToken": self.token,
                "nickname": self.name,
                "language": "en-US",
                "picture": "",
            },
            callback=self.join_room_callback,
        )

    async def turn(self, use_correct=False):
        best_word = ""
        for word in (correct if use_correct else unused):
            await asyncio.sleep(0)
            if use_correct:
                word = word.upper()
                if word not in unused:
                    continue
            if self.milestone["syllable"].upper() in word:
                if word in correct:
                    if not best_word:
                        best_word = word
                        if use_correct:
                            break
                else:
                    best_word = word
                    break
        if best_word:
            await self.gameSio.emit("setWord", (best_word, True))
            unused.remove(best_word)
        else:
            await self.sio.emit("chat", f"I don't know anything for {self.milestone['syllable']}.")

    def is_leader(self, peer_id: int = None):
        if peer_id is None:
            peer_id = self.peerId
        return peer_id == self.leaderPeerId

    async def events(self):
        @self.sio.event
        async def connect():
            print(self.name, "connected to JKLM!")

        @self.gameSio.event
        async def connect():
            print(self.name, "connected to Bomb Party!")

        @self.gameSio.event
        async def setup(data):
            self.leaderPeerId = data.get("leaderPeerId", -1)
            self.peerId = data.get("selfPeerId", -2)
            if self.peerId >= 0:
                human_ids.add(self.peerId)
            if self.is_leader():
                await self.gameSio.emit("setRulesLocked", False)
                await self.gameSio.emit(
                    "setRules",
                    {
                        "promptDifficulty": "custom",
                        # "customPromptDifficulty": -1,
                        # "customPromptDifficulty": 50,
                        "startingLives": 5,
                        "maxLives": 10,
                        "maxPromptAge": 1,
                        "minTurnDuration": 1,
                        # "minTurnDuration": 1,
                    },
                )
                await self.gameSio.emit("setRulesLocked", True)
            self.milestone = data["milestone"]
            await self.gameSio.emit("joinRound")

        @self.gameSio.event
        async def setLeaderPeer(peer_id):
            self.leaderPeerId = peer_id

        @self.gameSio.event
        async def nextTurn(peer_id, syllable, prompt_age):
            self.milestone["currentPlayerPeerId"] = peer_id
            self.milestone["syllable"] = syllable
            self.milestone["promptAge"] = prompt_age
            if self.milestone.get("currentPlayerPeerId", -3) == self.peerId:
                self.attempts = 0
                await self.turn()

        @self.gameSio.event
        async def setMilestone(milestone, time):
            self.milestone = milestone
            if milestone["name"] == "round":
                if milestone["currentPlayerPeerId"] == self.peerId:
                    await self.turn()
            elif milestone["name"] == "seating":
                await self.gameSio.emit("joinRound")
                if self.is_leader():
                    global unused
                    unused = words.copy()

        @self.gameSio.event
        async def failWord(peer_id, reason):
            if self.is_leader() and reason == "notInDictionary":
                cleaned = cleanup_word(latest_word)
                if cleaned.upper() in words and cleaned not in wrong:
                    wrong.append(cleaned)
                    print(f"{len(wrong)} incorrect:", cleaned)
                    with open("wrong.txt", "a") as f:
                        f.write(cleaned + "\n")
            if peer_id == self.peerId:
                self.attempts += 1
                await self.turn(self.attempts > 2)

        @self.gameSio.event
        async def correctWord(data):
            if self.is_leader():
                if data["playerPeerId"] not in human_ids:
                    try:
                        unused.remove(latest_word.upper())
                    except ValueError:
                        pass
                cleaned = cleanup_word(latest_word)
                if cleaned not in correct:
                    correct.append(cleaned)
                    print(f"{len(correct)} words:", cleaned)
                    with open("correct.txt", "a") as f:
                        f.write(cleaned + "\n")
            if self.peerId == data["playerPeerId"]:
                self.bonus_letters = data["bonusLetters"]

        @self.gameSio.event
        async def setPlayerWord(peer_id, word):
            if self.is_leader():
                global latest_word
                latest_word = word

        @self.sio.event
        async def chat(author, message):
            if message == "!ping":
                await self.sio.emit("chat", "pong")
            elif message == "!start" and self.peerId == self.leaderPeerId:
                await self.sio.emit("chat", "Ok")
                await self.gameSio.emit("startRoundNow")


async def main():
    global words, unused, correct, wrong, code, human_ids, latest_word
    words = set()
    for file in glob.glob('../lists/*.txt'):
        if "insane" in file:
            print(file)
            continue
        with open(file, "r") as f:
            words = words.union(set(f.read().split()))
            # for line in f:
            #     upper = line.upper()
            #     if upper not in words:
            #         words.append(upper)
    print(f"Loaded {len(words)} words.")
    words = [word.upper() for word in words if not (re.match(r"[^A-Za-z-']", word) or word.endswith("'s"))]
    words = list(set(words))
    random.shuffle(words)
    # words.sort(key=lambda word: len(set(word)), reverse=False)
    # words.sort(key=lambda word: 40 if "-" in word else len(set(word)), reverse=True)
    print(f"Loaded {len(words)} filtered words.")
    with open("correct.txt", "r") as f:
        correct = f.read().split()
    print(f"Loaded {len(correct)} correct words.")
    with open("wrong.txt", "r") as f:
        wrong = f.read().split()
    print(f"Loaded {len(wrong)} wrong words.")
    unused = words.copy()

    token = create_token()
    payload = {
        "name": "Humans only!",
        "isPublic": False,
        "gameId": "bombparty",
        "creatorUserToken": token,
    }
    headers = {"content-type": "application/json"}

    response = requests.request(
        "POST", "https://jklm.fun/api/startRoom", json=payload, headers=headers
    )
    data = response.json()
    code = data.get("roomCode")
    if not code:
        raise Exception
    print(f"https://jklm.fun/{code}")
    humans = []
    human_ids = set()
    latest_word = ""

    for x in range(8):
        if x != 0:
            token = create_token()
        humans.append(Human(token, data["url"], f"Human {x}"))
        await humans[-1].connect_and_join()
    # await humans[0].gameSio.emit("setRulesLocked", True)
    await humans[0].sio.wait()

if __name__ == '__main__':
    asyncio.run(main())
